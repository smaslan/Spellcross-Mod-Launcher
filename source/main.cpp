...
#include "forms/form_units_rand.h"
...
{		
    ...
    int win_y_size = ini.GetLongValue("STATE","win_y_size",750);
...
{
	// <wxFormsBuilder> - Section auto-inserted from 'forms.cpp' class 'FormMain' on 2026-09-11 19:38:43
	...
	mmSetup = new wxMenu();
	wxMenuItem* mmInstall;
	mmInstall = new wxMenuItem( mmSetup, wxID_MM_INSTALL, wxString( _("Install game") ) , wxEmptyString, wxITEM_NORMAL );
	mmSetup->Append( mmInstall );
	
	wxMenuItem* mmPatch;
	mmPatch = new wxMenuItem( mmSetup, wxID_MM_PATCH, wxString( _("Apply patch to SPELCROS.EXE") ) , wxEmptyString, wxITEM_NORMAL );
	mmSetup->Append( mmPatch );
	...
	mmRunSetupBat = new wxMenuItem( mmSetup, wxID_MM_RUN_SETUPBAT, wxString( _("Run game SETUP.BAT (sound)") ) , wxEmptyString, wxITEM_NORMAL );
	...
	wxMenuItem* mmCfgRandomizer;
	mmCfgRandomizer = new wxMenuItem( mmMod, wxID_MM_CFG_RANDOMIZER, wxString( _("Config units randomizer") ) , wxEmptyString, wxITEM_NORMAL );
	mmMod->Append( mmCfgRandomizer );
	
	wxMenuItem* mmCfgTreesRand;
	mmCfgTreesRand = new wxMenuItem( mmMod, wxID_MM_CFG_TREES_RAND, wxString( _("Config trees randomizer") ) , wxEmptyString, wxITEM_NORMAL );
	mmMod->Append( mmCfgTreesRand );
	...
	wxMenuItem* mmSaveCheckOrig;
	mmSaveCheckOrig = new wxMenuItem( msmSaveOrig, wxID_MM_SAVE_CHECK_ORIG, wxString( _("Check saves consistency") ) , wxEmptyString, wxITEM_NORMAL );
	msmSaveOrig->Append( mmSaveCheckOrig );
	...
	wxMenuItem* mmSaveEditOrig;
	mmSaveEditOrig = new wxMenuItem( msmSaveOrig, wxID_MM_SAVE_EDIT_ORIG, wxString( _("Savegame editor") ) , wxEmptyString, wxITEM_NORMAL );
	msmSaveOrig->Append( mmSaveEditOrig );
	...
	wxMenuItem* mmSaveCheckMod;
	mmSaveCheckMod = new wxMenuItem( msmSaveMod, wxID_MM_SAVE_CHECK_MOD, wxString( _("Check saves consistency") ) , wxEmptyString, wxITEM_NORMAL );
	msmSaveMod->Append( mmSaveCheckMod );
	...
	wxMenuItem* mmSaveEditMod;
	mmSaveEditMod = new wxMenuItem( msmSaveMod, wxID_MM_SAVE_EDIT_MOD, wxString( _("Savegame editor") ) , wxEmptyString, wxITEM_NORMAL );
	msmSaveMod->Append( mmSaveEditMod );
	
	mmSave->Append( msmSaveModItem );
	
	wxMenuItem* mmAutoCheckSave;
	mmAutoCheckSave = new wxMenuItem( mmSave, wxID_MM_AUTO_CHECK_SAVE, wxString( _("Auto check saves consistency") ) , wxEmptyString, wxITEM_CHECK );
	mmSave->Append( mmAutoCheckSave );
	...
	bSizer115->Add( bSizer78, 1, wxEXPAND, 5 );
	...
	m_staticText46 = new wxStaticText( this, wxID_ANY, _("Game engine version:"), wxDefaultPosition, wxDefaultSize, 0 );
	m_staticText46->Wrap( -1 );
	bSizer81->Add( m_staticText46, 0, wxRIGHT|wxLEFT, 5 );
	
	txtGameEngVersion = new wxTextCtrl( this, wxID_TXT_GAME_ENG_VER, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_READONLY );
	bSizer81->Add( txtGameEngVersion, 1, wxRIGHT|wxLEFT|wxEXPAND, 5 );
	...
	bSizer82->SetMinSize( wxSize( 200,-1 ) );
	
	bSizer82->Add( 0, 0, 1, wxEXPAND, 5 );
	
	
	bSizer80->Add( bSizer82, 0, wxEXPAND, 5 );
	
	
	bSizer114->Add( bSizer80, 0, wxEXPAND, 5 );
	...
	bSizer24->Add( chModPath, 1, wxBOTTOM|wxRIGHT|wxLEFT, 5 );
	...
	m_staticText42 = new wxStaticText( this, wxID_ANY, _("Options:"), wxDefaultPosition, wxDefaultSize, 0 );
	m_staticText42->Wrap( -1 );
	bSizer22->Add( m_staticText42, 0, wxRIGHT|wxLEFT, 5 );
	
	pgModOptions = new wxPropertyGrid(this, wxID_PG_MOD_OPTS, wxDefaultPosition, wxDefaultSize, wxPG_BOLD_MODIFIED|wxPG_DEFAULT_STYLE|wxPG_HIDE_CATEGORIES);
	bSizer22->Add( pgModOptions, 0, wxEXPAND|wxBOTTOM|wxRIGHT|wxLEFT, 5 );
	...
	bSizer122->Add( cbAllowCDmod, 0, wxBOTTOM|wxRIGHT|wxLEFT, 5 );
	...
	cbNoNight = new wxCheckBox( this, wxID_CB_NO_NIGHT, _("Disable night vision"), wxDefaultPosition, wxDefaultSize, 0 );
	bSizer122->Add( cbNoNight, 0, wxBOTTOM|wxRIGHT|wxLEFT, 5 );
	
	cbTreesRand = new wxCheckBox( this, wxID_CB_TREES_RAND, _("Randomize trees"), wxDefaultPosition, wxDefaultSize, 0 );
	bSizer122->Add( cbTreesRand, 0, wxBOTTOM|wxRIGHT|wxLEFT, 5 );
	
	m_staticText43 = new wxStaticText( this, wxID_ANY, _("Unit randomizer mode:"), wxDefaultPosition, wxDefaultSize, 0 );
	m_staticText43->Wrap( -1 );
	bSizer122->Add( m_staticText43, 0, wxRIGHT|wxLEFT, 5 );
	
	wxArrayString chRandomizeModeChoices;
	chRandomizeMode = new wxChoice( this, wxID_CH_RAND_MODE, wxDefaultPosition, wxDefaultSize, chRandomizeModeChoices, 0 );
	chRandomizeMode->SetSelection( 0 );
	bSizer122->Add( chRandomizeMode, 0, wxEXPAND|wxBOTTOM|wxRIGHT|wxLEFT, 5 );
	...
	mmSaveEditOrig->SetBitmaps(LoadSVGiconsBundle("IDR_EDIT"));
	mmSaveEditMod->SetBitmaps(LoadSVGiconsBundle("IDR_EDIT"));	
	mmSaveCheckOrig->SetBitmaps(LoadSVGiconsBundle("IDR_RELOAD"));
	mmSaveCheckMod->SetBitmaps(LoadSVGiconsBundle("IDR_RELOAD"));
	...
	mmInstall->SetBitmaps(LoadSVGiconsBundle("IDR_INSTALL"));
	mmPatch->SetBitmaps(LoadSVGiconsBundle("IDR_EDIT"));
	...
	mmCfgRandomizer->SetBitmaps(LoadSVGiconsBundle("IDR_EDIT"));
	mmCfgTreesRand->SetBitmaps(LoadSVGiconsBundle("IDR_EDIT"));
	...
	// auto check save games consistency before run
	mmAutoCheckSave->Check(m_ini->GetBoolValue("SETUP","auto_check_saves",false));

	// try load last units randomizer preset
	auto c_rand_path = m_ini->GetValue("PATH","units_randomizer_preset");
	if(c_rand_path)
		m_randomizer.LoadIni(c_rand_path);

	// try load trees randomzier presets
	m_tree_randomizer.LoadINIpresets(m_ini,"PATH","tree_randomizer_preset");
	...
	// units randomizer mode
	chRandomizeMode->Append("Off");
	chRandomizeMode->Append("Map rules");
	chRandomizeMode->Append("Local rules");
	chRandomizeMode->Select(0);
	chRandomizeMode->Select(m_ini->GetLongValue("SETUP","units_randomize",0));
	...
	cbNoNight->SetValue(m_ini->GetBoolValue("SETUP","disable_night_vission",false));
	cbTreesRand->SetValue(m_ini->GetBoolValue("SETUP","trees_randomize",false));
	...
	Bind(wxEVT_COMMAND_CHOICE_SELECTED,&FormMain::OnChangeExe,this,wxID_CH_SPELL_EXE);
	...
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnInstallGame,this,wxID_MM_INSTALL);
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnPatchExe,this,wxID_MM_PATCH);
	...
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnUnitRandomizeConfig,this,wxID_MM_CFG_RANDOMIZER);
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnTreesRandomizeConfig,this,wxID_MM_CFG_TREES_RAND);
	...
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnSaveEdit,this,wxID_MM_SAVE_EDIT_ORIG);
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnSaveEdit,this,wxID_MM_SAVE_EDIT_MOD);
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnSaveCheck,this,wxID_MM_SAVE_CHECK_ORIG);
	Bind(wxEVT_COMMAND_MENU_SELECTED,&FormMain::OnSaveCheck,this,wxID_MM_SAVE_CHECK_MOD);
	...
	Bind(wxEVT_COMMAND_CHECKBOX_CLICKED,&FormMain::OnModSaveSource,this,wxID_CB_MOD_SAVES);
	Bind(wxEVT_PG_CHANGED,&FormMain::OnModOptionChange,this,wxID_PG_MOD_OPTS);
...
void FormMain::OnClose(wxCloseEvent& ev)
{
	...
	m_ini->SetValue("PATH","units_randomizer_preset",m_randomizer.m_path.string().c_str(),"; Last units randomize preset");

	// trees randomizer preset(s)
	m_tree_randomizer.SaveINIpresets(m_ini,"PATH","tree_randomizer_preset");
	...
	m_ini->SetBoolValue("SETUP","auto_check_saves",GetMenuBar()->FindItem(wxID_MM_AUTO_CHECK_SAVE)->IsChecked(),"; auto check save games consistency before run");
	...
	m_ini->SetLongValue("SETUP","units_randomize",chRandomizeMode->GetSelection(),"; Units randomization");
	m_ini->SetLongValue("SETUP","trees_randomize",cbTreesRand->GetValue(),"; Trees randomization");
	m_ini->SetBoolValue("SETUP","move_mod_saves",cbModSaves->GetValue(), "; Move save games with the mod");	
	m_ini->SetBoolValue("SETUP","disable_night_vission",cbNoNight->GetValue(),"; Disable night vission in missions");

	

	// default window size
	m_ini->SetLongValue("STATE","win_x_size",GetSize().GetWidth());
	m_ini->SetLongValue("STATE","win_y_size",GetSize().GetHeight());
	m_ini->SetBoolValue("STATE","win_maximize",IsMaximized());
...
{
    auto form = new FormAbout(this,str_ver_label);
...
// game installer
void FormMain::OnInstallGame(wxCommandEvent& event)
{
	// prompt
	{
		wxMessageDialog dial(this,string_format("INSTALLING SPELLCROS\n\nThis is simple game installer that will unpack game data from archive INSTALL.DTA located in DATA folder of original Spellcross CD.\nYou have to select the INSTALL.DTA file from your CD path and target directory to install the game.\n\nContinue to install?"),"Install Spellcross game ...",wxYES_NO | wxICON_QUESTION);
		if(dial.ShowModal() != wxID_YES)
			return;
	}
	
	// select CD path
	auto cd_path = GetPathChoiceLastPath(chSpellCdPath);
	auto dir = std::filesystem::path(cd_path).wstring();
	auto name = "INSTALL.DTA";
	wxFileDialog openFileDialog(this,"Select game installer DTA archive",dir,name,"Spellcross (INSTALL.DTA)|INSTALL.DTA",
	...
	if(saveDirDialog.ShowModal() == wxID_CANCEL)
		return;	
	std::filesystem::path install_path = saveDirDialog.GetPath().ToStdWstring();
	...
	// list EXE
	CheckExeVersion();
	ListSpellExecutables(install_path,chSpellExec);
...
// game patcher
void FormMain::OnPatchExe(wxCommandEvent& event)
{
	std::filesystem::path spell_dir = GetPathChoiceLastPath(chSpellPath);
	std::string spell_exe = chSpellExec->GetStringSelection().ToStdString();
	if(spell_dir.empty() || !std::filesystem::exists(spell_dir))
	{
		wxMessageBox(string_format("No or invalid Spellcross path selected!"),"Error",wxICON_ERROR);
		return;
	}
...
// on save editor launch
void FormMain::OnSaveEdit(wxCommandEvent& event)
{
	if(FindWindowById(wxID_FORM_SAVE_EDIT))
		return;
	form_save_edit = new FormSaveEdit(this,wxID_FORM_SAVE_EDIT);
	if(!form_save_edit)
		return;	
	
	auto spell_dir = GetPathChoiceLastPath(chSpellPath);
	if(event.GetId() == wxID_MM_SAVE_EDIT_ORIG)
	{
		// original game data		
		if(spell_dir.empty())
		{
	...
	else
	{
		...
		if(mod_path.empty())
		{
			wxMessageBox("Spellcross mod path not selected! Cannot run save edit for modded version without mod path.","Error",wxICON_ERROR);
			return;
		}
		
		SpellMod::Config mod_config;
		mod_config.allow_cd_mod = cbAllowCDmod->GetValue();
		mod_config.move_saves = cbModSaves->GetValue();
		mod_config.force_write = false;
		mod_config.mod_path = mod_path;
		...
		form_save_edit->SetCommonPath(common_fs_path,true);
	}

	// set saves directory
	if(spell_dir.empty())
	{
		wxMessageBox("Spellcross directory not selected! Cannot continue.","Error",wxICON_ERROR);
		return;
	}
	auto save_dir = std::filesystem::path(spell_dir) / "SAVE";
	if(event.GetId() == wxID_MM_SAVE_EDIT_MOD && cbModSaves->GetValue())
	{
		auto mod_path = GetPathChoiceLastPath(chModPath);
		if(mod_path.empty())
		{
	...
	form_save_edit->SetSaveDir(save_dir);

	form_save_edit->Show();
}


// on units randomizer launch
void FormMain::OnUnitRandomizeConfig(wxCommandEvent& event)
{
	...
	if(mod_path.empty())
	{
		wxMessageBox("Spellcross mod path not selected! Cannot run unit randomizer without mod path.","Error",wxICON_ERROR);
		return;
	}
	...
	// make editor gui
	form_unit_rand = new FormUnitRand(this,m_randomizer,wxID_FORM_UNIT_RAND);
	if(!form_unit_rand)
		return;
...
// on trees randomizer launch
void FormMain::OnTreesRandomizeConfig(wxCommandEvent& event)
{
	if(FindWindowById(wxID_FORM_TREES_RAND))
		return;
	...
	if(mod_path.empty())
	{
		wxMessageBox("Spellcross mod path not selected! Cannot run trees randomizer without mod path.","Error",wxICON_ERROR);
		return;
	}
	...
	if(!make_dir_path)
	{
		wxMessageBox(string_format("Spellcross mod MAKE directory for mod \"%s\" not defined! Cannot proceed with trees randomzier.",mod_path),"Error",wxICON_ERROR);
		return;
	}
	
	// find terrains (and common.fs)
	std::vector<std::shared_ptr<FSarchive>> terrains;
	std::vector<std::string> terr_names = {"COMMON","T11","PUST","DEVAST"};
	for(auto &terr: terr_names)
	{	
		auto fs_name = string_format("%s.FS",terr);
		auto fs_path = make_dir_path->path / fs_name;
		if(!std::filesystem::exists(fs_path))
		{
			fs_path = std::filesystem::path(mod_config.spell_dir) / "data" / fs_name;
			if(!std::filesystem::exists(fs_path))
			{
				wxMessageBox(string_format("Spellcross %s archive not found at path \"%s\"! Cannot proceed with trees randomzier.",fs_name,fs_path),"Error",wxICON_ERROR);
				return;
			}
		}		

		std::shared_ptr<FSarchive> arch;
		try{
			arch = std::make_shared<FSarchive>(fs_path);
		}catch(const std::runtime_error& error) {
			wxMessageBox(string_format("Spellcross %s archive loading failed at path \"%s\"! Cannot proceed with trees randomzier.",fs_name,fs_path),"Error",wxICON_ERROR);
			return;
		}
		
		terrains.push_back(arch);
	}

	// make rules editor gui
	form_trees_rand = new FormTreeRand(this,wxID_FORM_TREES_RAND);
	if(!form_trees_rand)
		return;
	...
	// run it
	form_trees_rand->Show();
}


// make mod config from object config
SpellMod::Config ProcTh::MakeModConfig()
{
	SpellMod::Config mod_config;
	mod_config.randomize = m_config.unit_randomizer;
	...
	mod_config.options = m_config.options;
	mod_config.ver = m_config.ver;
	mod_config.rand_rules = m_config.randomize_rules;
	mod_config.trees_rand = m_config.trees_rand;
	mod_config.trees_rand_rules = m_config.trees_rand_rules;
	mod_config.no_night_vission = m_config.no_night_vission;
	mod_config.check_maps = m_config.check_maps;

	return(mod_config);
}

// get saves path from current config
std::filesystem::path ProcTh::GetSavesPath(bool is_mod,bool after_swap)
{
	std::filesystem::path path;

	if(is_mod && m_config.move_saves && !after_swap)
	{
		// assume saves in mod folder
		if(m_config.mod_path.empty() || !std::filesystem::exists(m_config.mod_path))
			return(path);
		auto save_dir = m_config.mod_path.parent_path() / "save";
		if(std::filesystem::exists(save_dir))
			path = save_dir;
		return(path);		
	}

	// assume save in game directory
	if(m_config.spell_dir.empty())
		return(path);
	auto save_dir = m_config.spell_dir / "SAVE";
	if(std::filesystem::exists(save_dir))
		path = save_dir;
	return(path);
}

// get common.fs path from current config
std::filesystem::path ProcTh::GetCommonFsPath(bool is_mod, bool after_swap)
{
	std::filesystem::path path;

	if(is_mod && !after_swap)
	{
		// assume common.fs in mod make folder
		auto mod_config = MakeModConfig();

		SpellMod mod;
		if(mod.LoadDEF(mod_config))
			return(path);
		auto make_dir_path = mod.GetPath("MAKE");
		if(!make_dir_path)
			return(path);
		auto fs_path = make_dir_path->path / "common.fs";
		if(std::filesystem::exists(fs_path))
			path = fs_path;
		return(path);
	}
	...
	if(m_config.spell_dir.empty())
		return(path);
	auto fs_path = m_config.spell_dir / "DATA" / "COMMON.FS";
	if(std::filesystem::exists(fs_path))
		path = fs_path;
	return(path);
}
...
{			
	SpellMod::Config mod_config = MakeModConfig();
	SpellMod mod;
	...
	{		
		...
		if(action == Action::CHECK_SAVE)
		{
			SetStatusCallback("Checking save games ...");
			ConsoleStringCallback("Checking save games consistency ... ");
...
// dialog callback
int ProcTh::DialogCallback(std::string message, std::string title)
{
	auto evt = new wxThreadEvent(wxEVT_THREAD,FormMain::wxID_PROC_THREAD);
	evt->SetInt(Event::DIALOG);
	evt->SetString(message);
	
	std::atomic<int> res = -1;
	evt->SetPayload<std::atomic<int>*>(&res);
	auto handler = m_parent->GetEventHandler();
	handler->QueueEvent(evt);
...
// save check panel
int ProcTh::CheckSaves(FormSaveCheck::Params &par)
{
	auto evt = new wxThreadEvent(wxEVT_THREAD,FormMain::wxID_PROC_THREAD);
	evt->SetInt(Event::SAVE_CHECK);
	
	std::atomic<int> res = -1;			
	evt->SetPayload<std::pair<std::atomic<int>&,FormSaveCheck::Params&>>({res,par});
	auto handler = m_parent->GetEventHandler();
	handler->QueueEvent(evt);
...
void FormMain::OnBuildModEvent(wxThreadEvent& event)
{
	...
	if(what == ProcTh::Event::DONE)
	{
		...
		// ###note: this is required to recover from DOSbox fullscreen mode
		Layout();
	}
	...
	else if(what == ProcTh::Event::DIALOG)
	{
		// dialog		
		FormDialog dlg(m_parent,event.GetString(),FormMain::wxID_FORM_DIALOG,"Checking save games...");
		auto res = event.GetPayload<std::atomic<int>*>();
		res->store(dlg.ShowModal());
	}
	else if(what == ProcTh::Event::SAVE_CHECK)
	{
		// save check		
		auto payload = event.GetPayload<std::pair<std::atomic<int>&,FormSaveCheck::Params&>>();		
		FormSaveCheck dlg(this,payload.second,FormMain::wxID_FORM_SAVE_CHECK);
		if(!payload.second.result)
		{
			// nothing to do
			payload.first.store(1);
			return;
		}
		payload.first.store(dlg.ShowModal());
	}
...
void FormMain::SetControlsState(bool busy)
{
	...
	for(auto &id: list)
	{
		if(std::find(exclude_list.begin(),exclude_list.end(),id) != exclude_list.end())
			continue;
...
// change exe
void FormMain::OnChangeExe(wxCommandEvent& event)
{
	CheckExeVersion();
}
...
// get current setup for processing thread
ProcTh::Params FormMain::GetProcThParams()
{
	ProcTh::Params config;
	...
	config.unit_randomizer = (SpellMod::RandomizerMode)chRandomizeMode->GetSelection();
	config.randomize_rules = m_randomizer;
	config.move_saves = cbModSaves->GetValue();
	config.trees_rand = cbTreesRand->GetValue();
	config.trees_rand_rules = m_tree_randomizer;
	config.state_ini_path = std::filesystem::path(GetExecutableDir()) / str_mod_state_ini_none;
	config.run_mode = chRunMode->GetStringSelection();
	config.force_build = false;
	config.options = m_mod_options;
	config.ver = CheckExeVersion();
	config.check_saves = GetMenuBar()->FindItem(wxID_MM_AUTO_CHECK_SAVE)->IsChecked();
	config.no_night_vission = cbNoNight->GetValue();
	config.check_maps = true;
	return(config);
}
...
void FormMain::OnBuildMod(wxCommandEvent& event)
{
	...
	// load setup
	ProcTh::Params config = GetProcThParams();
	config.force_build = evt_id == wxID_MM_MOD_BUILD_FORCE;
	if(config.ver == SpellLaunch::EngineVersion::NONE)
	{
	...
	if(evt_id == wxID_MM_MOD_BUILD || evt_id == wxID_MM_MOD_BUILD_FORCE)
	{
		...
		/*if(config.check_saves)
			actions.push_back(ProcTh::Action::CHECK_SAVE);*/
	}
...
// run saves consistency check
void FormMain::OnSaveCheck(wxCommandEvent& event)
{
	auto evt_id = event.GetId();
	bool is_mod = evt_id == wxID_MM_SAVE_CHECK_MOD;
	...
	if(is_mod && config.mod_path.empty())
	{
		wxMessageBox("Mod path not defined!","Checking save games consistency...",wxICON_ERROR);
		return;
	}
	if(!is_mod && config.spell_dir.empty())
	{
		wxMessageBox("Spellcross path not defined!","Checking save games consistency...",wxICON_ERROR);
		return;
	}
...
// change mod save sourcefolder
void FormMain::OnModSaveSource(wxCommandEvent& event)
{
	FillModOptions();
}
...
// check if currently selected mod has save in its folder
bool FormMain::CheckModSaves()
{
	auto mod_path = GetPathChoiceLastPath(chModPath);
	if(mod_path.empty())
		return(false);	
	auto save_dir = std::filesystem::path(mod_path).parent_path() / "save";
	return(SpellSave::CheckSaves(save_dir));
}


void FormMain::ConsoleBuffer(std::string str)
{
	m_console_buffer.push_back(str);
}
// clear mod options
void FormMain::ClearModOptions()
{
	pgModOptions->Clear();
...
void FormMain::FillModOptions()
{
	...
	m_console_buffer.clear();
	SpellMod mod;
	mod.SetConsoleOutCb(bind(&FormMain::ConsoleBuffer,this,std::placeholders::_1));
	if(mod.LoadDEF(mod_config))
	{
		textOutput->Clear();
	...
	m_mod_options = mod.m_options;

	// fill options
	pgModOptions->Freeze();
	pgModOptions->Clear();
	for(auto &opt: m_mod_options)
	{		
		if(opt.isEnum())
			pgModOptions->Append(new wxEnumPropertyExt(opt.description,wxT(""),MapToPGenumChoices(opt.GetEnumMap()),&opt.value));
		else
			pgModOptions->Append(new wxIntPropertyExt(opt.description,wxT(""),&opt.value));
	}
	pgModOptions->Thaw();
	pgModOptions->FitColumns();
	//pgModOptions->SetAutoLayout(true);
	setPGsize(pgModOptions,7,4);
...
SpellLaunch::EngineVersion FormMain::CheckExeVersion()
{
	...
	SpellLaunch::GameEngineVersion(spell_dir, spell_exe.ToStdString(),ver);
	txtGameEngVersion->SetValue(ver.version_name);
	
	LogFile::Write("done\n");

	return(ver.engine_ver);
}
