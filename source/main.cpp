...
{
	// <wxFormsBuilder> - Section auto-inserted from 'forms.cpp' class 'FormMain' on 2026-09-01 18:02:44
...
void FormMain::OnSaveEdit(wxCommandEvent& event)
{
	...
	else
	{
		...
		if(!make_dir_path)			
		{
			wxMessageBox(string_format("Spellcross mod MAKE directory for mod \"%ls\" not defined! Cannot continue.",mod_path.c_str()),"Error",wxICON_ERROR);
			return;
		}
		auto common_fs_path = make_dir_path->path / "common.fs";
		if(!std::filesystem::exists(common_fs_path))
		{
			wxMessageBox(string_format("Spellcross COMMON.FS archive not found at mod MAKE path \"%ls\"! Make sure the mod was built before using modded save editor!\nSave editor will now use original game COMMON.FS in case selected mod does not produce modded version of COMMON.FS.",common_fs_path.wstring().c_str()),"Warning",wxICON_WARNING);
			if(spell_dir.empty())
			{
				wxMessageBox("Spellcross directory not selected!","Error",wxICON_ERROR);
				return;
	...
	if(save_dir.empty() || !std::filesystem::exists(save_dir))
	{
		...
		{
			wxMessageBox(string_format("Spellcross SAVE folder \"%ls\" does not exit! Cannot continue.",save_dir.wstring().c_str()),"Error",wxICON_ERROR);
			return;
		}
	}
