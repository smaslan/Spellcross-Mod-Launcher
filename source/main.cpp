...
{
	// <wxFormsBuilder> - Section auto-inserted from 'forms.cpp' class 'FormMain' on 2026-08-16 18:06:03
...
void FormMain::OnInstallGame(wxCommandEvent& event)
{
	// prompt
	{
		wxMessageDialog dial(this,string_format("INSTALLING SPELLCROS\n\nThis is simple installer that will unpack game data from INSTALL.DTA archive located in DATA folder of Spellcross CD. \nYou have to select the INSTALL.DTA file path and target directory to install.\n\nContinue to install?"),"Install Spellcross game ...",wxYES_NO | wxICON_QUESTION);
		if(dial.ShowModal() != wxID_YES)
			return;
	}
	...
	// auto select CD path
	auto cd_dir = dta_path.parent_path();
...
SpellLaunch::EngineVersion FormMain::CheckExeVersion()
{
	...
	if(ver == SpellLaunch::EngineVersion::CZE)
		txtGameEngVersion->SetValue("Czech");
	else if(ver == SpellLaunch::EngineVersion::ENG)
